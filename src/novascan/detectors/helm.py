...
def _walk_values(data: dict, prefix: str, resources: dict):
    ...
    if "resources" in data and isinstance(data["resources"], dict):
        ...
        if requests or limits:
            ...
            resources[name] = {
                "cpu_request": parse_cpu(requests.get("cpu", limits.get("cpu", "0"))),
                "memory_request_gb": parse_mem_gb(requests.get("memory", limits.get("memory", "0"))),
    ...
    if "persistence" in data and isinstance(data["persistence"], dict):
        ...
        if p.get("enabled", True) and p.get("size"):
            ...
            if name not in resources:
                resources[name] = {}
            resources[name]["storage_gb"] = parse_mem_gb(p["size"])
    ...
    if "storage" in data and isinstance(data["storage"], dict):
        ...
        if s.get("size"):
            ...
            if name not in resources:
                resources[name] = {}
            resources[name]["storage_gb"] = parse_mem_gb(s["size"])
...
def _parse_templates(templates_dir: Path) -> dict[str, dict]:
    ...
    for f in templates_dir.rglob("*.yaml"):
        ...
        if cpu_matches or mem_matches:
            ...
            cpu_vals = [parse_cpu(c) for c in cpu_matches if parse_cpu(c) > 0]
            mem_vals = [parse_mem_gb(m) for m in mem_matches if parse_mem_gb(m) > 0]

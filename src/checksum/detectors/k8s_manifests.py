        ...
        try:
            ...
            for doc in yaml.safe_load_all(content):
                ...
                if "kind" not in doc:
                    continue
                manifest_count += 1
                _extract_resources(doc, total_cpu, total_memory_gb)
...
def _extract_resources(doc: dict, cpu_total: float, mem_total: float):
    """Walk a manifest for container resource requests."""
    containers = []
    spec = doc.get("spec", {})
    if "template" in spec:
